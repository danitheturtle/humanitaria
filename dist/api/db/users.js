...
const usersApi = dbRef => ({
  ...
  getUserWithVerifiedEmail: async email => {
    const [emailVerifiedUser] = await dbRef.from("users").where("email", email).where("email_verified", true).select('*');
  ...
  createUser: async userSeed => {
    ...
    const [newUser] = await dbRef.table('users').insert(newUserData).returning('*');
    return newUser;
  },
  deleteUser: async userData => {
    ...
    const [deletedUser] = await dbRef.from('users').where('uid', uid).del(['uid']).returning('*');
    return deletedUser;
  },
  updateUser: async userData => {
    ...
    const [updatedUser] = await dbRef.table("users").where("uid", uid).update(userData).returning("*");
    return updatedUser;
  }
});
